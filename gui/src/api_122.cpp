...
static Api122Support* GetApiSupport() {
  auto* support = dynamic_cast<Api122Support*>(wxTheApp);
  assert(support && "cannot downcast app to Api122Support");
  return support;
}
...
void HostApi122::RegisterApiEventCallback(
  ...
  auto& msg_types_callbacks = m_api_support->GetApiEventsCallbacks();
  if (callback)

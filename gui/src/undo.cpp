...
Undo::Undo(UndoHandler* undo_handler) {
  m_undo_handler = undo_handler;
  depthSetting = 10;
  stackpointer = 0;
...
void doUndoAppendWaypoint(UndoAction* action, UndoHandler* uh) {
  RoutePoint* point = (RoutePoint*)action->before[0];
  Route* route = (Route*)action->after[0];

  bool noRouteLeftToRedo = false;
  if ((route->GetnPoints() == 2) && (uh->GetRouteState() == 0))
    noRouteLeftToRedo = true;

  g_pRouteMan->RemovePointFromRoute(point, route, uh->GetRouteState());
  ...
  GuiEvents::GetInstance().on_waypoint_update.Notify();
  uh->UndoAppendWaypoint(route, noRouteLeftToRedo);
}

void doRedoAppendWaypoint(UndoAction* action, UndoHandler* uh) {
  RoutePoint* point = (RoutePoint*)action->before[0];
  Route* route = (Route*)action->after[0];

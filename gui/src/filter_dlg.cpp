...
/** Two state button showing either an 'edit' or 'done' icon. */
...
/** EditIcon and a text entry allows for user text input.*/
...
public:
  DescriptionPanel(wxWindow* parent, NavmsgFilter& filter,
    ...
    auto text_entry = new TextEntryPanel(
        this, "Description", filter.m_description, std::move(on_update));
    vbox->Add(text_entry, wxSizerFlags().Border().Expand());
    SetSizer(vbox);
    Fit();
    Show();
  }
